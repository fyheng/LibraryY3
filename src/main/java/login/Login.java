	...
	public Login() {
		...
		lblNewLabel_1.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if(usernametxt.getText().length()==0||passwordtxt.getText().length()==0) {
					JOptionPane.showMessageDialog(contentPane,"Please insert username and password","Fail",JOptionPane.WARNING_MESSAGE);  
				}
			}
		});
		...
		showpassword.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				((JPasswordField) passwordtxt).setEchoChar(showpassword.isSelected() ? '\u0000' : (Character) UIManager.get("PasswordField.echoChar"));
			}
		});
